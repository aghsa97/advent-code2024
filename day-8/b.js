    ...
    for (let j = i + 1; j < nodes.length; j++) {
      if (curr === nodes[j][1]) {
        ...
        for (let m = 0; m < list.length; m++) {
          const r_t = parseInt(curr_row) + diff_row * m;
          const c_t = parseInt(curr_col) + diff_col * m;
        ...
        for (let m = 0; m < list.length; m++) {
          const r_b = parseInt(curr_row) - diff_row * m;
          const c_b = parseInt(curr_col) - diff_col * m;
